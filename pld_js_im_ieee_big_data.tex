...
\end{align}
where $\bar{x}_q$ represents the average across all mappers of the $q$th element of the vector $\bar{x}\in\mathbf{R}^n$, in other words: $\bar{x}_q = \sum_{j=1}^N x_{q_j}$, and similarly for $\bar{u}$.  The first elements, $\bar{x}_1$ and $\bar{u}_1$, correspond to the intercept and are not regularized, therefore we must treat them differently from the other values in the vector.  $k$ is the iteration number, and $\rho^k$ is the penalty parameter for iteration $k$, which is used as the step size.
