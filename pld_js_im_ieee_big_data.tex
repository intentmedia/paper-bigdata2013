...
The ADMM algorithm does not require that data fit into memory on a single machine, instead computing the solution to an intermediate optimization problem on subsets of the training data and combining the intermediate solutions to find a consensus solution at each iteration \cite{boyd}. Although the iterations must be run sequentially, the time-consuming and memory-intensive computation of the intermediate optimization problems is distributed to enable solving the problem in Equation \ref{eq:formula} \todo{I removed the equations here, but it may make sense to move up the first two (non-distributed) equations from the Implementation section} on a large dataset.  Given a Hadoop-based implementation of the algorithm, the size of memory can be scaled with the dataset just by adding more machines to the Hadoop cluster.
...
\todo{may make sense to split subsections A and B into two sections, one for "Problem Formulation" and one for "Implementation"}
